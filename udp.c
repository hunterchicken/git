...
udp_register_add_locator(void *data, struct map_entry * e, int ex_info)
{
	...
	if( (_fncs & _FNC_XTR) && lisp_te && e->pe && ex_info){
		...
		while(ptr != &e->pe->tail){
			...
			loc_te->R = e->r;
			if(_debug == LDEBUG){
				fprintf(OUTPUT_STREAM, "\t•[rloc=TE, priority=%u, weight=%u, m_priority=%u, m_weight=%u, r=%d, L=%d, p=%d]\n", \
					pe->priority, \
					pe->weight, \
					pe->m_priority, \
					pe->m_weight, \
...
{
	if(_debug == LDEBUG)
		fprintf(OUTPUT_STREAM, "Unknown error\n");
		...
		const struct prefix * eid)
{
	...
	//rec = (union map_request_record_generic_lcaf *)itr_rloc;
...
udp_init_socket()
{
	...
	setsockopt(skfd6, IPPROTO_IPV6, IPV6_V6ONLY, &only_bind_ipv6, sizeof(only_bind_ipv6));		
	...
	switch(lh->type){
		...
		case LISP_TYPE_MAP_REQUEST:
			...
			if(_fncs & _FNC_XTR){
				...
				if(rt <= 0){
					if(_debug == LDEBUG)
						fprintf(OUTPUT_STREAM, "Not correct map-request.....Ignore!\n");
		...
		case LISP_TYPE_ENCAPSULATED_CONTROL_MESSAGE:
			...
			if(rt <= 0){
				if(_debug == LDEBUG)
					fprintf(OUTPUT_STREAM, "Not a map-request.....Ignore!\n");
			...
			if(_fncs & _FNC_XTR){
				...
				if(generic_process_request(pke, &udp_fct) <= 0){
					if(_debug == LDEBUG)
...
udp_get_pk(int sockfd, socklen_t slen)
{
	...
	bzero(buf, PKBUFLEN);	
	if ( (pk_len = recvfrom(sockfd, buf, PKBUFLEN, 0, (struct sockaddr *)&(ssk.sa), &slen)) < 0){
		perror("recvfrom");
		return -1;
	}
...
_register(void *data)
{
	...
	rcount = lcm->record_count;
	if(_debug == LDEBUG){
		fprintf(OUTPUT_STREAM, "LCM: <type=%u, P=%u, M=%u, rcount=%u, nonce=0x%x - 0x%x, key id=%u, auth data length=%u\n", \
				lcm->lisp_type,
				lcm->proxy_map_reply, \
				lcm->want_map_notify,
				rcount, \
			...
			case LCAF_AFI:
				...
				while( (void *)hop < barr){
					switch(ntohs(hop->rloc.afi)){
						...
						default:
							if(_debug == LDEBUG)
								fprintf(OUTPUT_STREAM, "unsuported family rlocs:%d\n",ntohs(hop->rloc.afi));
							return 0;	
					}					
				}
				break;
	...
	for (; ;){
		...
		while(ptr != &etr_db->tail){
			...
			if(lisp_te && (_fncs & _FNC_XTR)){
				...
				udp_register_add_record(rpk_ex, &node->p, mflags->ttl, lcount, mflags->version, mflags->A, mflags->act);
			}
			_iter = l->head.next;
			if(_iter == &l->tail){
				ptr = ptr->next;
		...
		while(ptr != &xtr_ms->tail){
			...
			if(ms->proxy && lisp_te && (_fncs & _FNC_XTR)){
				hr = (struct map_register_hdr *)rpk_ex->buf;
				buflen = rpk_ex->buf_len;				
			}
			else{	
			...
			/*Send */
			if(ms->proxy && lisp_te && (_fncs & _FNC_XTR))
				udp_register_terminate(rpk_ex, (union sockunion *) ms);
			else	
				udp_register_terminate(rpk, (union sockunion *) ms);
			ptr = ptr->next;
		}
		
		//wake up each 1 minute to send map-register
		//hexout(rpk_ex->buf,rpk_ex->buf_len);
		if(lisp_te && (_fncs & _FNC_XTR))
			_free_rpl_pool_place(rpk_ex, _rm_rpl);
		else
			_free_rpl_pool_place(rpk, _rm_rpl);
		sleep(60);
	}
...
send_mr_ddt(uint32_t idx)
{
	...
	if(mr_lookups[idx].active){
		...
		if(_debug == LDEBUG)	{
			fprintf(OUTPUT_STREAM, "send Map-Request ");
			fprintf(OUTPUT_STREAM, "to %s:%d\n", sk_get_ip(&servaddr, ip),sk_get_port(&servaddr));
			fprintf(OUTPUT_STREAM, "Sending packet... ");
		}
...
mr_new_lookup(void *data,struct communication_fct * fct,struct db_node *rn)
{
    ...
    //memcpy(&mr_lookups[i].last_eid, &eid, sizeof(struct prefix));
...
mr_event_loop(void *context)
{
	...
	for (;;) {
        ...
        for (i = 0; i < MAX_LOOKUPS; i++) {
            if (!(mr_lookups[i].active)) continue;
			if(mr_lookups[i].count > MAX_COUNT){
				free(mr_lookups[i].orgi_pkg);
				list_destroy(mr_lookups[i].rlocs,NULL);
				close(mr_lookups[i].rx);
				close(mr_lookups[i].rx6);
				mr_lookups[i].active = 0;
			}
			
            deadline.tv_sec = mr_lookups[i].start.tv_sec + (mr_lookups[i].count +1) * timeout; 
